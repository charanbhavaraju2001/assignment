//C program to convert Infix expression to a Prefix expression
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
#define MAX 25

//Global
char infix[25],prefix[25],stack[25];
int top = -1;

void infix_to_prefix();
void push(int);
char pop();
int precedence(char);
char strrev(char);
void reverse(char *);

void main(){
	printf("Enter the Infix Expression\n");
	scanf("%s",infix);
	infix_to_prefix();
}

//pushes the infix char to the stack
void push(int pos){
	if(pos==MAX-1){
		printf("Stack Overflow\n");
	}
	else{
		top++;
		stack[top] = infix[pos];
	}
}

//pops the top of the stack
char pop()
{
	char ch;
	if(top < 0)
	{
	printf("\nSTACK UNDERFLOW\n");
	exit(0);
	}
	else
	{
	ch = stack[top];
	stack[top] = '\0';
	top--;
	return(ch);
	}
	return 0;
}

void infix_to_prefix()
{
int i = 0,j = 0;
//to calculate the prefix we reverse the infix expr
reverse(infix);
//traversing through each character using a loop
while(infix[i] != '\0')
{
// if an alphabet is found then copy it to the output string
	if(infix[i] >= 'a' && infix[i] <= 'z')
	{
	prefix[j] = infix[i];
	j++;
	i++;
	}
	// In the reversed string, closing bracket will be found first so put it in the stack
	else if(infix[i] == ')')
	{
	push(i);
	i++;
	}
	// if an opening bracket is found, then
	else if(infix[i] == '(') /* when closing bracket is found */
		{
			if(infix[i] == '(')
			{
				while(stack[top] != ')') /* pop till opening bracket is found */
				{
				prefix[j] = pop();
				j++;
				}
				pop();
				i++;
			}
		}
	else
	{
	// if the stack if empty, then we simply put the operator in stack
		if(top == -1)
		{
		push(i);
		i++;
		}
	// if the precedence of operator is less than the stack top then
	else if( precedence(infix[i]) < precedence(stack[top]))
	{
	prefix[j] = pop();  // pop the stack top and add it to the prefix string
	j++;
	// if you have an operator that has precedence greater than operator
	while(precedence(stack[top]) > precedence(infix[i])){
		prefix[j] = pop();    // Pop the operator
		j++;
		if(top < 0) {
		break;
		}
	}
	push(i);
	i++;
	}
	// if the precedence of operator is greater than or equal to the stack top 
	else if(precedence(infix[i]) >= precedence(stack[top]))
	{
	push(i);  //  Push it onto the stack
	i++;
	}}
}
// At the end remove all the operators from the stack
while(top != -1)
{
	prefix[j] = pop();
	j++;
}
// Reverse the final string before output
reverse(prefix);
prefix[j] = '\0';
printf("Equivalent Prefix Notation : %s ",prefix);
}


int precedence(char alpha)
{
if(alpha == '+' || alpha =='-')
{
return(1);
}
if(alpha == '*' || alpha =='/')
{
return(2);
}
return 0;
}

//user defined strrev function to reverse the string
void reverse(char *s)
{
   int length, c;
   char *begin, *end, temp;
 
   length = strlen(s);
   begin  = s;
   end    = s;
 
   for (c = 0; c < length - 1; c++)
      end++;
 
   for (c = 0; c < length/2; c++)
   {        
      temp   = *end;
      *end   = *begin;
      *begin = temp;
 
      begin++;
      end--;
   }
}
